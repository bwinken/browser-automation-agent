...
| `ADMIN_USERNAME` | Auto-create admin user on startup | — |
| `ADMIN_PASSWORD` | Admin user password | — |
| `INITIAL_INVITE_CODES` | Generate N invite codes on first run | `3` |
...
### Authentication
- Registration requires a valid **invite code** (generated by admin)
- All task endpoints require `Authorization: Bearer <api_key>` header
- In `DEV_MODE=true`, auth is bypassed (uses first user in DB)
- On first startup with `ADMIN_USERNAME` set, admin user + invite codes are auto-created

### Endpoints

```
POST /api/users/register          — Register with invite code → returns api_key
POST /api/users/login             — Login → returns api_key
POST /api/users/admin/invite      — (Admin) Generate new invite code
GET  /api/users/admin/invites     — (Admin) List all invite codes
...
Login/register page with invite code → then chat-based interface with three panels:
